...
if($position == 3)
{
    $under_sub_admin_id = 1;

    $getUnderSubAminId = "SELECT under_sub_admin_id FROM users WHERE id = '".$id."'";
    $rowGetUnderSubAminId= mysqli_query($con, $getUnderSubAminId);
    $runGetUnderSubAminId = mysqli_fetch_assoc($rowGetUnderSubAminId);
    //$runGetUnderSubAminId['under_sub_admin_id'];

    $update1 = "UPDATE leads SET assign_subadmin_id = '".$under_sub_admin_id."'  WHERE assign_subadmin_id = '".$runGetUnderSubAminId['under_sub_admin_id']."'";
    mysqli_query($con, $update1);

    $update = "UPDATE users SET name = '".$name."',email = '".$email."', contact = '".$phone."', under_sub_admin_id = '".$under_sub_admin_id."', position = '".$position."' WHERE id = '".$id."'";
}
else
{
    $update = "UPDATE users SET name = '".$name."',email = '".$email."', contact = '".$phone."', position = '".$position."' WHERE id = '".$id."'";
}
...
<head>
    ...
    <title>Super Admin | View Users | Bharat Finn</title>
            ...
            <form method="GET">
                <div class="col-md-12">
                    <table align="center" class="" style="table-layout: fixed;">
                        <tr>
                        <td>
                            <input type="text" name="user_referral_id" class="form-control" placeholder="Enter User Referral ID" required>
                        </td>
                        <td>
                            <input type="submit" name="search" class="btn btn-primary" value="Search">
                            <?php
                                $sql2=" SELECT * FROM users WHERE login_id = '".$_GET['user_referral_id']."' ";
                                $query2=mysqli_query($con,$sql2);
                                $run2=mysqli_fetch_assoc($query2);
                            ?>
                        </td>
...
<?php if($_GET['user_id'] =='' && $_GET['user_id'] || $_GET['user_referral_id'] !='' && $_GET['user_referral_id'])
{?>
...
<?php if($_GET['position_id'] !='' && $_GET['position_id'] || $_GET['user_referral_id'] !='' && $_GET['user_referral_id'])
{?>
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    ...
                                    <h7><?php echo $msg; ?></h7>                                
                                        ...
                                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tbody>
                                                <?php
                                                    $x = 1;
                                                    $fetch = "SELECT * FROM users WHERE position = '".$_GET['position_id']."' OR login_id = '".$_GET['user_referral_id']."' ORDER BY id DESC";
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php 
                                                            if ($run['referenceBy'] != NULL)
                                                            {
                                                                echo $run['referenceBy'];
                                                            }
                                                            else
                                                            {
                                                                echo "---";
